...
protected:
    ...
    std::expected<int, FileSystemStatus> call_allocate_inode(FileSystem &fs)
    {
    ...
    std::expected<Inode, FileSystemStatus> call_get_inode(FileSystem &fs, int inode_id)
    {
...
TEST_F(InodeManagerTest, allocate_inode)
{
    ...
    {
        auto result = call_allocate_inode(fs);
